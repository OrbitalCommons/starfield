...
use crate::planet_spectra::albedo::{AlbedoKind, SpectralAlbedo};
use crate::planet_spectra::body::SpectralBody;

/// The 1995 low-resolution table, vendored verbatim from the PDS volume.
const EMBEDDED_LOW_1995: &str = include_str!("../../data/planet-spectra/1995low.tab");
...
fn adopt_legacy_cache(store: &Datastore, product: Product) -> bool {
    adopt_legacy_cache_from(
        store,
        product,
        &starfield_core::data::source_utils::cache_dir().join(LEGACY_CACHE_SUBDIR),
    )
}
...
fn adopt_legacy_cache_from(store: &Datastore, product: Product, legacy_dir: &Path) -> bool {
    ...
    if !starfield_core::data::source_utils::file_exists_and_not_empty(&legacy) {
        return false;
    }
...
mod tests {
    ...
    fn hermetic_child() {
        let legacy = starfield_core::data::source_utils::cache_dir()
