...
use starfield_core::data::source_utils::datastore::{
...
use starfield_surfaces::reflectance_library::Endmember;

use crate::planet_maps::grid::MapGrid;
use crate::planet_maps::map::PhotometricBand;
...
pub struct MapProduct {
    ...
    /// they can drive an [`crate::planet_maps::AlbedoMap`]; see the crate docs.
    pub monochrome: bool,
    /// Provenance note.
...
mod tests {
    ...
    fn planetary_products_use_the_usgs_grid_convention() {
        use crate::planet_maps::grid::{Latitude, Longitude, RowOrder};
    ...
    fn earth_uses_geodetic_latitude_and_a_centred_seam() {
        ...
        use crate::planet_maps::grid::{Latitude, Longitude};
    ...
    fn pinned_urls_are_reachable() {
        for p in PRODUCTS {
            let client = starfield_core::data::source_utils::build_http_client(60).unwrap();
            let response = client
                .head(p.url())
                .send()
