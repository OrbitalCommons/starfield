...
use starfield_surfaces::reflectance_library::{Endmember, EndmemberMix};

use crate::planet_maps::grid::{Latitude, Longitude, MapGrid, RowOrder};
use crate::planet_maps::map::{SampleFootprint, SurfaceSampler, MU_FLOOR};
...
impl AbundanceTier {
    ...
    pub fn from_bytes(b: &[u8]) -> Result<Self> {
        ...
        let grid = MapGrid {
            ...
            flattening: crate::planet_maps::products::EARTH_WGS84_FLATTENING,
        };
    ...
    ///
    /// Identical policy to [`crate::planet_maps::AlbedoMap::select_level`]: select from the
    ...
    ///
    /// The tier equivalent of [`crate::planet_maps::AlbedoMap::sample_area`], and the reason
...
pub const MOON_TIER_GZ: &[u8] = include_bytes!("../../data/planet-maps/moon_albedo_0p1deg.bin.gz");
...
pub const MARS_TIER_GZ: &[u8] = include_bytes!("../../data/planet-maps/mars_albedo_0p1deg.bin.gz");
...
pub const EARTH_TIER_GZ: &[u8] =
    include_bytes!("../../data/planet-maps/earth_endmembers_0p25deg.bin.gz");
...
mod tests {
    ...
    fn the_trait_gives_one_interface_for_both_backings() {
        use crate::planet_maps::map::{PhotometricBand, SurfaceSampler};
        use crate::planet_maps::MapGrid;

        let t = tier();
        let m = crate::planet_maps::AlbedoMap::new(
    ...
    fn reproduced_band_albedo(t: &AbundanceTier) -> f64 {
        let lib =
            starfield_surfaces::reflectance_library::ReflectanceLibrary::load_embedded().unwrap();
