...
use starfield::catalogs::gaia::{Downloader, Dr1, Dr2, Dr3};
use starfield::catalogs::mast::{DataProduct, MastClient};
use starfield::surfaces::planet_maps::PRODUCTS;
use starfield_core::data::source_utils::{adopt_legacy, artifact::materialize, sha256_bytes};
...
fn mast_product_identity_and_mirror_fit_validation() {
    ...
    let source =
        starfield_core::data::source_utils::artifact_from_url(&opaque.resolve_url().unwrap())
            .unwrap();
...
fn shared_url_identity_and_typed_errors_match_starfield() {
    use starfield_core::data::source_utils::{artifact_from_url, datastore_error};
    let kernel_url =
    ...
    assert_eq!(
        ...
        starfield_core::data::kernel_artifact("de421.bsp")
            .unwrap()
            .key
    );
    ...
    assert_eq!(
        artifact_from_url(url).unwrap().key,
        starfield_core::data::url_artifact(url).unwrap().key
    );
    ...
    assert!(matches!(
        error,
        starfield_core::StarfieldError::Datastore(starfield_datastore::DatastoreError::Config(_))
    ));
}
...
fn live_mirror_serves_core_pds_and_gaia_from_empty_client_caches() {
    use starfield::surfaces::planet_spectra::{KarkoschkaTable, Product};
    ...
    for _ in 0..2 {
        ...
        let lsk = starfield_core::data::download_or_cache_with(&store, "naif0012.tls").unwrap();
        assert_eq!(
