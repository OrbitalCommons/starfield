...
use crate::gaia::common::catalog::{GaiaCatalog, MemoryResidentCatalog};
use crate::gaia::common::cone::Cone;
use crate::gaia::common::lazy::LazyLoadingCatalog;
use crate::gaia::download::Downloader;
use crate::gaia::dr1::entry::{Dr1Entry, TgasBlock};
use crate::gaia::dr1::schema::Dr1;
...
impl Dr1Catalog {
    ...
    ///
    /// See [`crate::gaia::dr1::supplement`] for the data provenance, the synthetic
    ...
    /// [`crate::gaia::dr1::supplement::decode_supplement_hip`].
    pub fn augment_missing(&mut self, mag_limit: f64) -> Result<usize> {
        let rows = crate::gaia::dr1::supplement::parse_embedded_supplement()?;
        let mut added = 0;
        for row in &rows {
            ...
            self.0
                .insert(crate::gaia::dr1::supplement::supplement_row_to_entry(row));
            added += 1;
        }
        Ok(added)
    }
